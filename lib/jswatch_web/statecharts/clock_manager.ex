defmodule JswatchWeb.ClockManager do
  ...
  # Intervalos de tiempo para las funciones recurrentes
  ...
  # Ignorar mensajes mientras se edita
  @impl true
  def handle_info(message, state) do
    case message do
      ...
      _ -> raise " #{inspect(message)}"
    end
  end



  # Avanzar el tiempo del reloj cada segundo
 ...
 # Presionar botón izquierdo (alarma)
 ...
 # En edición: botón derecho cambia segmento (hora/minuto/segundo)
  ...
  defp verify_button_hold(press_start_time, adjustment_type, %{is_key_down: true, key_press_timer: key_timer, activity_mode: :Viewing, main_time: time_val, alarm_setting: alarm_val, ui_pid: display_pid} = state) do
    if press_start_time == key_timer do
...
#Mostrar que edita el tiempo
        case adjustment_type do
          :time_adjust ->
            GenServer.cast(display_pid, {:set_time_display, "CLOCK"})
...
#Mostrar que edita la alarma
          :alarm_adjust ->
            GenServer.cast(display_pid, {:set_time_display, "ALARM"})
            Process.send_after(self(), :blink_animation, @blink_cycle_duration)
  ...
  #INCREMENTAR LA SECCION SEG/MIN/HORA
